...
<!-------------------------------------------------- 导航栏 ------------------------------------------------->
	<div class="menu">
		...
		<div class="nav">
			...
			<div><a href="study.do" id="now">学无止境</a>
				<ul class="xwzj" style="display:none;">
					<li><a href="study.do?id=1" <c:if test="${param.id==1||param.subject=='心得笔记'}">id="now"</c:if>>心得笔记</a></li>
					<li><a href="study.do?id=2" <c:if test="${param.id==2||param.subject=='前端知识'}">id="now"</c:if>>前端知识</a></li>
					<li><a href="study.do?id=3" <c:if test="${param.id==3||param.subject=='后台研发'}">id="now"</c:if>>后台研发</a></li>
					<li><a href="study.do?id=4" <c:if test="${param.id==4||param.subject=='其他知识'}">id="now"</c:if>>其他知识</a></li>
				</ul>
			</div>
...
<!-- -----------------------------------------------主体内容----------------------------------------------------- -->
	<div class="container">
		...
		<h1 class="t_nav">
			...
			<a href="study.do" <c:if test="${param.id==null&&param.subject==null}">class="n2"</c:if>
			<c:if test="${param.id!=null||param.subject!=null}">class="n1"</c:if>>学无止境
			<c:if test="${param.id==null&&param.subject==null}">(${fn:length(lists)})</c:if></a>
			<c:if test="${param.id==1||param.subject=='心得笔记'}"><a href="" class="n2">心得笔记(${fn:length(lists)})</a></c:if>
			<c:if test="${param.id==2||param.subject=='前端知识'}"><a href="" class="n2">前端知识(${fn:length(lists)})</a></c:if>
			<c:if test="${param.id==3||param.subject=='后台研发'}"><a href="" class="n2">后台研发(${fn:length(lists)})</a></c:if>
			<c:if test="${param.id==4||param.subject=='其他知识'}"><a href="" class="n2">其他知识(${fn:length(lists)})</a></c:if>
		</h1>
		<!-- -----------------博客的主要内容展示(左)-------------------- -->
		<div class="blog_show">
